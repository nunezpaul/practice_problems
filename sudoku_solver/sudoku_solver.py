...
class Cell(object):
    ...
    def has_singleton(self):
        valid_nums = self.missing_square_nums.intersection(self.missing_row_nums, self.missing_col_nums)
        if len(valid_nums) == 1:
            self.val = valid_nums.pop()
            self._update_possibilities(self.val)
            return True
        return False

    def _update_possibilities(self, only_valid_num):
        for category in (self.missing_row_nums, self.missing_col_nums, self.missing_square_nums):
            if only_valid_num in category:
...
if __name__ == '__main__':
    ...
    sudoku_puzzle = Board(board)
    print(len(sudoku_puzzle.missing_cells))
